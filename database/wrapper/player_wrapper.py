...
class PlayerWrapper:
    ...
    def get_player_games(self, session, id, id_list=False):
        if id_list:
            ...
            return session.query(PlayerGame).join(Game).filter(Game.players.contains(cast(id, postgresql.ARRAY(String)))).filter(
                PlayerGame.player == id[0])
        else:
            return session.query(PlayerGame).filter(PlayerGame.player == id).filter(
                PlayerGame.game is not None)

    def get_player_games_paginated(self, session, id, id_list=False, page=0):
        query = self.get_player_games(session, id, id_list=id_list)

        return self.get_paginated_match_history(query, page=page, id_list=id_list)
    ...
    def get_total_games(self, session, id, id_list=False):
        return self.get_player_games(session, id, id_list=id_list).count()
