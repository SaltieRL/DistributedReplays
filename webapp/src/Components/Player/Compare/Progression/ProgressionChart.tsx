...
export class ProgressionChart extends React.PureComponent<Props> {
    public render() {
        return <Line data={this.getChartData()} options={this.getChartOptions()} />
    }

    private readonly getChartData = (): ChartData => {
        return {
            datasets: this.props.playerPlayStyleProgressions.map((playerPlayStyleProgression, i) => {
                return {
                    label: playerPlayStyleProgression.player.name,
                    data: playerPlayStyleProgression.playStyleProgressionPoints.map((playStyleProgressionPoint) => {
                        return {
                            x: playStyleProgressionPoint.date as any,
                            y: playStyleProgressionPoint.dataPoints.find(
                                (dataPoint) => dataPoint.name === this.props.field
                            )!.average
                        } as ChartPoint
                    }),
                    fill: false,
