...
import { Player, PlayStyleProgressionPoint } from "src/Models"
...
export class PlayerProgressionCharts extends React.PureComponent<Props, State> {
    ...
    public componentDidUpdate(prevProps: Readonly<Props>, prevState: Readonly<State>) {
        ...
        if (
            this.state.timeUnit !== prevState.timeUnit ||
        ...
        return (
            <>
                <Grid item xs={12} md={10} xl={8} style={{ textAlign: "center" }} container spacing={16}>
                    ...
                    <Grid item xs={12} sm={6} lg={3}>
                        <FormControl>
                            ...
                            <Select value={this.state.timeUnit} onChange={this.handleTimeUnitChange} autoWidth>
                ...
                {this.state.selectedFields.map((field) => {
                    return (
                        <Grid item xs={12} md={6} lg={5} key={field} style={{ height: 400 }}>
    ...
    private readonly updateFields = () => {
        ...
        const fields = _.uniq(
            _.flatMap(this.state.playStyleProgressions, (playStyleProgression) =>
                _.flatMap(playStyleProgression, (playStyleProgressionPoint) =>
                    playStyleProgressionPoint.dataPoints.map((dataPoint) => dataPoint.name)
                )
            )
        )
